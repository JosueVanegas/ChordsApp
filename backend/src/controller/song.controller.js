...
export const getSongs = async (req, res) => {
  try {
    ...
    if (data.length == 0)
      return res.status(200).json({ message: "there's no songs yet" });
...
export const getSong = async (req, res) => {
  try {
    ...
    if (!_id)
      return res
        .status(200)
        .json({ message: "Missing id, the id param is required" });
    const data = await Song.findById({ _id });
    console.log(data);
    if (!data) return res.status(200).json({ message: "query failed" });
    if (data.length == 0)
      return res.status(200).json({ message: "song dont founded" });
    return res.status(200).json(data);
  } catch (error) {
...
export const putSong = async (req, res) => {
  try {
    ...
    const request = {
      ...
      updatedBy: body.updatedBy || "unknow",
    };
...
export const deleteSong = async (req, res) => {
  try {
    ...
    if (!song[0].affectedRows)
      return res
        .status(200)
        .json({ message: "the server can´t deleted the song selected" });
